...
# [All the same helper functions from your original script remain unchanged]
    ...
    try:
        with gpu_context(gpu_id):
            ...
            for task_id, task_data in batch_tasks:
                ...
                # More aggressive cache clearing for stability
                if len(results) % 5 == 0:
                    torch.cuda.empty_cache()
                    gc.collect()
...
def main():
    ...
    parser.add_argument('--start-from', type=int, default=None, help='Start from a specific index (for resuming)')
    parser.add_argument('--num-tasks', type=int, default=144, help='Number of tasks to process from the end')
    parser.add_argument('--resume', action='store_true', help='Resume from previously completed tasks')
    ...
    # Create output directory for last 144 tasks
    ...
    # Get the last 144 tasks
    start_index = len(task_ids) - args.num_tasks
    if args.start_from is not None:
        start_index = args.start_from
    
    task_ids = task_ids[start_index:start_index + args.num_tasks]
    print(f"Processing tasks from index {start_index} to {start_index + args.num_tasks}")
    print(f"First task ID: {task_ids[0]}, Last task ID: {task_ids[-1]}")
    
    # Check for previously completed tasks if resuming
    completed_task_ids = set()
    if args.resume and os.path.exists(os.path.join(output_dir, 'all_results.json')):
        with open(os.path.join(output_dir, 'all_results.json'), 'r') as f:
            previous_results = json.load(f)
            completed_task_ids = {r['task_id'] for r in previous_results if r['status'] == 'success'}
            print(f"Found {len(completed_task_ids)} previously completed tasks")
    
    # Filter out completed tasks
    if completed_task_ids:
        task_ids = [tid for tid in task_ids if tid not in completed_task_ids]
        print(f"Remaining tasks to process: {len(task_ids)}")
